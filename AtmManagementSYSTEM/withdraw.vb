...
Public Class withdraw
            ...
            Try
                ...
                If count = 1 Then
                    ...
                    If initialbal <= txtAmt.Text Then
                        MsgBox("Insufficient balance ", MsgBoxStyle.Critical, "Insufficient balance")
                    ElseIf initialbal > 0 Then

                        If amt > 20000 Then
                            MsgBox("You cant Withdraw more than 10000 rupees", MsgBoxStyle.Critical, "Alert")
                        ElseIf amt < 200 Then
                            MsgBox("You cant Withdraw less than 200 rupees", MsgBoxStyle.Critical, "Alert")
    ...
    Public Sub wit_statement()
        wit_date = Transaction.label5.Text
        mysqlconn = New MySqlConnection
        ...
        Try
            mysqlconn.Open()
            Dim query As String
            query = " insert into atm.withdraw_statement (acc_no,w_date,w_amt) values ('" & lblAcc.Text & "','" & wit_date & "','" & amt & "')"
            Command = New MySqlCommand(query, mysqlconn)
            reader = Command.ExecuteReader
            mysqlconn.Close()
        Catch ex As MySqlException
    ...
    Public Sub withdrawal()
        ...
        Try
            ...
            MsgBox("Amount Withdrawn Succesfully, Confirmation sent via email ", MsgBoxStyle.Information, "Withdraw")
            getemail()
            emailsender()
            mysqlconn.Close()
            Me.Close()
    ...
    Public Sub emailsender()
        ...
        Try
            ...
            eMessage.Subject = "Withdraw"
            eMessage.Priority = MailPriority.High
            eMessage.Body = "Hey " & lblName.Text & " you account number  " & lblAcc.Text & "  has been debited rs." & txtAmt.Text & " on " & lblDate.Text & " Available balance is " & total & ""
